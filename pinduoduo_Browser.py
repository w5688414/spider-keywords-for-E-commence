...
class MainWindow(QWidget):
    ...
    def setupUI(self):
        ...
        self.web_browser.load(QUrl('https://youhui.pinduoduo.com/'))
    ...
    def callback(self, html):
        ...
        # pq模块解析网页源代码
        doc = pq(html)
        good_items=doc('#__next > div > div.site > div > div.card-wrapper .goods-detail-card-wrapper').items()
        keywords=doc('#__next > div > div.search-bar > div.content > div > input[type=text]').attr('value')
        print(keywords)
        ...
        for item in good_items:
            ...
            products=[]
            # print('====')
            # #__next > div > div.site > div > div.card-wrapper > a:nth-child(2)
            good_title = item.find('p.double-line-desc.bottom-10').text()
            good_price = item.find('p.after-coupon-price-type2').text()
            good_url=item.attr.href
            deal=item.find('.sale-number').text()
            # print(good_title)
            # print(good_price)
            # print(good_url)
            # print(deal)
            products=[good_title,good_price,deal,good_url]
            print(products)
        ...
        csv_name=os.path.join('pinduoduo',keywords+'.csv')
...
def output_to_csv(list_data,csv_name):
    column_name = ['good_title', 'good_price','deal','good_url']
